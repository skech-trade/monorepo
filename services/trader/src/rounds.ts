...
import type { Exec, Position, VenueFill } from "./executor";
...
export type Round = {
  ...
  startedAt: number;
  accountIndex: number;
  /** BTC each trade in this round opens. */
  quantity: number;
  ...
  problem: string | null;
  /** Kept for rounds recorded before trades had ids. */
  openedWith?: number;
};
...
export class Rounds {
  ...
  /** The exits each running round watches. Memory only: a restored round never resumes. */
  ...
  async resume(round: Round, exec: Exec) {
    ...
    const open = this.openTrade(round) ?? round.trades.findLast((t) => t.status === "closing") ?? null;
    for (const t of round.trades) if (t !== open && (t.status === "open" || t.status === "opening" || t.status === "closing")) t.status = "closed";
    ...
    if (round.exits) this.spec.set(round.id, { pts: round.pts, stake: round.stake, leverage: round.leverage, seconds: round.seconds, exits: round.exits });
    round.problem = null;
    round.status = round.untracked || !round.segments.length ? "closing" : "running";
    this.publish(round);
    if (round.status === "running") await this.step(round);
  ...
  async edit(id: string, pts: Pt[], seconds = this.live.get(id)?.seconds ?? 0): Promise<Round> {
    ...
    if (!shape || shape.flat) throw Error("Draw a clear prediction before trading.");
    const frozen = this.now() + this.lead(exec) + (this.options.guardMs ?? 150);
  ...
  async skip(id: string, segmentId: string, skipped: boolean): Promise<Round> {
    const round = this.running(id);
    const exec = this.execs.get(id)!;
    round.segments = setSkipped(round.segments, segmentId, skipped, this.now() + this.lead(exec) + (this.options.guardMs ?? 150));
    await this.step(round);
    void this.save(round);
    return round;
  }
  ...
  async closeExisting(exec: Exec) {
    const existing = this.all().find((r) => r.accountIndex === exec.accountIndex && r.status !== "done");
    if (existing) {
      this.attach(existing.id, exec);
    ...
    const round: Round = {
      id: crypto.randomUUID(), status: "running", outcome: null, entry: held.avgEntry, stake: 0, leverage: 0, seconds: 0, startedAt: now,
      accountIndex: exec.accountIndex, quantity: Math.abs(held.size), size: held.size, unrealised: held.unrealised, realised: 0, net: null, pnlReady: false,
      pts: [], segments: [], trades: [{ id: tradeId, dir: held.size > 0 ? 1 : -1, size: Math.abs(held.size), status: "open", pnl: 0, entry: held.avgEntry }],
      orders: [], fills: [], untracked: true, timing: { requestedAt: now, closeRequestedAt: now }, problem: null,
    };
    this.accounts.add(exec.accountIndex);
    this.live.set(round.id, round);
    this.attach(round.id, exec);
  ...
  private async step(round: Round): Promise<void> {
    ...
    this.timers.set(round.id, setTimeout(() => void this.step(round).catch((e) => this.fail(round, e)), wake));
  }
  ...
  private async dispatch(round: Round, exec: Exec, cur: Trade | null, want: Run, lead: number): Promise<boolean> {
    ...
    if (cur) {
      ...
      cur.status = "closing";
      if (size > EPS) {
        const order: Order = { id: String(this.orderId()), tradeId: cur.id, kind: "close", side: cur.dir > 0 ? "sell" : "buy", size, reduceOnly: true, dueAt, requestedAt, filled: 0, pnl: 0, status: "sending" };
    ...
    if (want.dir !== 0) {
      ...
      const order: Order = { id: String(this.orderId()), tradeId: id, kind: "open", side: want.dir > 0 ? "buy" : "sell", size: sizeStep(market, round.quantity), reduceOnly: false, dueAt, requestedAt, filled: 0, pnl: 0, status: "sending" };
    ...
    this.publish(round);
    try {
      const sent = await exec.submit(
        orders.map((o) => ({ clientOrderIndex: BigInt(o.id), size: o.size, isAsk: o.side === "sell", reduceOnly: o.reduceOnly })),
        round.leverage,
      );
      orders.forEach((o, i) => {
        ...
        o.ackAt = sent.ackAt;
        o.via = sent.via;
        o.hash = sent.hashes[i + (sent.withLeverage ? 1 : 0)] ?? sent.hashes[i];
      ...
      if (!unknown) {
        ...
        if (round.orders.filter((o) => o.status !== "rejected").length > 0 && n < 3) {
          ...
          this.clearTimer(round.id);
          this.timers.set(round.id, setTimeout(() => void this.step(round), 400));
  ...
  private onFill(round: Round, f: VenueFill, pushed = true) {
    ...
    const fillId = String(f.trade_id_str ?? f.trade_id);
    if (seen.has(fillId)) return;
    const side = f.ask_account_id === round.accountIndex ? "ask" : f.bid_account_id === round.accountIndex ? "bid" : null;
    if (!side) return;
    const clientId = String(f[`${side}_client_id_str`] ?? f[`${side}_client_id`] ?? "");
    ...
    round.fills.push({ id: fillId, orderId: order.id, at: f.receivedAt, venueAt: f.timestamp, buy: side === "bid", price, size });
  ...
  private onPosition(round: Round, p: Position) {
    ...
    if (round.status === "running") {
      ...
      if (Math.abs(p.size) > round.quantity * 2 + this.market().minBase) return this.fail(round, Error("Position exceeds this round’s target size."));
      if (spec && round.net !== null) {
        if (spec.exits.lose !== null && round.net <= -Math.abs(spec.exits.lose)) {
          round.outcome = "stop";
          void this.finish(round);
        } else if (spec.exits.gain !== null && round.net >= Math.abs(spec.exits.gain)) {
      ...
      const cur = this.openTrade(round);
      if (cur && this.settled(round) && !this.inflight.has(round.id) && Math.abs(p.size) < EPS) {
        cur.status = "failed";
        round.problem = "An order did not fill; retrying at the market.";
        void this.step(round);
      }
    }
    this.publish(round);
  }
  ...
  private async complete(round: Round) {
    const exec = this.execs.get(round.id);
    if (!exec) return;
    const flat = () => Math.abs(exec.position()?.size ?? 0) < EPS;
    const accounted = () => round.orders.every((o) => o.status === "filled" || o.status === "rejected");
    try {
      ...
      let done = await this.until(round, exec, () => flat() && accounted(), this.options.flatMs ?? 6000);
      if (!done && !flat()) {
        // Still holding after the wait: close what the venue says is left, once.
        await this.sendClose(round, exec);
        done = await this.until(round, exec, () => flat() && accounted(), this.options.flatMs ?? 6000);
      }
      // The venue's own fill history is the figure the round is booked at, pushed or not.
      if (flat()) await this.reconcile(round, exec);
      if (flat()) {
        for (const t of round.trades) if (t.status === "closing" || t.status === "open" || t.status === "opening") t.status = "closed";
        round.status = "done";
  ...
  private async sendClose(round: Round, exec: Exec) {
    const cur = this.openTrade(round) ?? round.trades.findLast((t) => t.status === "closing") ?? null;
    const held = exec.position();
    const size = held && this.settled(round) ? Math.abs(held.size) : Math.max(held ? Math.abs(held.size) : 0, cur ? this.closeSize(round, exec, cur) : 0);
    if (size < EPS) return;
    const requestedAt = this.now();
    ...
    const order: Order = { id: String(this.orderId()), tradeId, kind: "close", side: dir > 0 ? "sell" : "buy", size: sizeStep(this.market(), size), reduceOnly: true, dueAt: requestedAt, requestedAt, filled: 0, pnl: 0, status: "sending" };
    if (cur) {
      cur.status = "closing";
      cur.closeOrderId = order.id;
    }
    ...
    this.publish(round);
    try {
      const sent = await exec.submit([{ clientOrderIndex: BigInt(order.id), size: order.size, isAsk: order.side === "sell", reduceOnly: true }]);
      Object.assign(order, { sentAt: sent.sentAt, ackAt: sent.ackAt, via: sent.via, hash: sent.hashes[0] });
      if (order.status === "sending") order.status = "acked";
  ...
  private until(round: Round, exec: Exec, done: () => boolean, ms: number) {
  ...
  private async booked(round: Round) {
    ...
    if (!exec) return;
    try {
      this.applyVenueFills(round, await exec.fillsSince(round.startedAt - 60_000));
      this.publish(round);
    } catch {
   ...
   */
  private applyVenueFills(round: Round, fills: Fill[]) {
    const now = this.now();
    ...
    for (const f of fills) {
      ...
      seen.add(id);
      const client = String(f[`${side}_client_id_str`] ?? f[`${side}_client_id`] ?? "");
  ...
  private async reconcile(round: Round, exec: Exec) {
    ...
    for (let i = 0; i < 6; i++) {
      try {
        this.applyVenueFills(round, await exec.fillsSince(round.startedAt - 60_000));
        for (const o of round.orders) if (o.status === "uncertain" && o.filled === 0) o.status = "rejected";
        this.tally(round);
  ...
  private async replay(round: Round) {
    try {
      const response = await fetch(`${this.options.feedUrl ?? process.env.TRADE_FEED_URL ?? "http://localhost:3210"}/bars?n=1200`, { signal: AbortSignal.timeout(3000) });

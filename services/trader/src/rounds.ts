...
export type RoundSpec = { market?: string; pts: Pt[]; stake: number; leverage: number; seconds: number; exits: { lose: number | null; gain: number | null } };
...
export class Rounds {
  ...
  private acked(o: Order, sent: Sent, hash: string | undefined) {
  ...
  async resume(round: Round, exec: Exec) {
    ...
    if (round.exits) this.spec.set(round.id, { market: round.market, pts: round.pts, stake: round.stake, leverage: round.leverage, seconds: round.seconds, exits: round.exits });
    round.problem = null;
  ...
  private async dispatch(round: Round, exec: Exec, cur: Trade | null, want: Run, lead: number): Promise<boolean> {
    ...
    this.publish(round);
    try {
      const sent = await exec.submit(orders.map(request), round.leverage);
      orders.forEach((o, i) => {
        // A leverage update signed into the same batch takes the first hash.
        this.acked(o, sent, sent.hashes[i + (sent.withLeverage ? 1 : 0)] ?? sent.hashes[i]);
  ...
  private async complete(round: Round) {
    ...
    const settled = () => flat() && round.orders.every((o) => o.status === "filled" || o.status === "rejected");
  ...
  private async sendClose(round: Round, exec: Exec) {
    ...
    this.publish(round);
    try {
      const sent = await exec.submit([request(order)]);
      this.acked(order, sent, sent.hashes[0]);

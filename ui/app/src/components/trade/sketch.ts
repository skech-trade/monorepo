...
/**
 * The drawn line as a trade.
 *
 * Pure functions, no React. The canvas turns a finger into points, this turns
 * points into the three prices and two dollar figures the sheet shows, and the
 * clock turns candles plus legs into money. Same rules as the landing's
 * `draw-canvas.tsx` and `levelsFor`, so what someone practised on skech.trade
 * is what they get here.
 */

/** A point of the drawn line: position across the forecast window, 0 at now
    and 1 at the edge, and the price it sits at. Never a pixel. */

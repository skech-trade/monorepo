...
const EMPTY: Profile = { name: null, balance: null, ready: true, setName: async () => null, refresh: () => undefined };
...
export function useProfile(address: string | null): Profile {
  ...
  return { name, balance, ready: answered === address, setName, refresh: () => setToken((n) => n + 1) };
}
